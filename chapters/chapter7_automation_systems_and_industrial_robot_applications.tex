...
\subsection{บทนำ: จากหุ่นยนต์หนึ่งตัวสู่ระบบอัตโนมัติในโรงงาน}
...
\subsubsection{ระดับของระบบอัตโนมัติที่เกี่ยวข้อง}
...
\subsubsection{หลักคิด “กระบวนการ–ชิ้นงาน–ระบบ–คน”}
...
\subsection{Robot Cell: แนวคิด องค์ประกอบ และการออกแบบ}

\subsubsection{นิยาม Robot Cell}
...
|  +---------------- Safety Fence ----------------------+   |
|  |                                                    |   |
|  | Conveyor IN -> Sensor -> Robot + EOAT -> Conveyor  |   |
|  |                         |                          |   |
|  |                    Fixture / Jig                   |   |
|  |                         |                          |   |
|  |                  Camera + Lighting                 |   |
...
\subsubsection{องค์ประกอบของ Robot Cell}
...
\subsubsection{ขั้นตอนการออกแบบ Robot Cell}
...
\begin{itemize}
    ...
    \item มาตรการลดความเสี่ยงตามลำดับ: ออกแบบให้ปลอดภัย \(\rightarrow\) ป้องกันทางวิศวกรรม \(\rightarrow\) ขั้นตอนปฏิบัติและการฝึกอบรม
\end{itemize}
...
\subsubsection{ตัวอย่างเพิ่มเติมเพื่อการเรียนรู้: ตรวจสอบ Payload และ Margin}
...
\subsubsection{อัตราการผลิตเชิงทฤษฎี}
...
\subsubsection{อัตราการผลิตจริงและประสิทธิภาพ}
...
\subsubsection{ตัวอย่างเพิ่มเติมเพื่อการเรียนรู้: Cycle Time และ Throughput}
...
\subsubsection{แนวทางลด Cycle Time โดยไม่ลดความปลอดภัย}
...
\subsection{การประยุกต์ใช้งานหุ่นยนต์ในกระบวนการอุตสาหกรรม}

\subsubsection{งานเชื่อมอาร์ก (Arc Welding)}
...
\begin{table}[htbp]
    \centering
    \small
    \begin{tabularx}{\textwidth}{L{0.18\textwidth} L{0.20\textwidth} Y Y}
        \hline
...
\subsubsection{งานเชื่อมจุด (Resistance Spot Welding)}
...
\subsubsection{งานหยิบจับและวาง (Pick and Place)}
...
\subsubsection{งานจัดเรียงพาเลท (Palletizing)}
...
\subsubsection{งานประกอบชิ้นงาน (Assembly)}
...
\subsubsection{SCARA Robot ในงานประกอบอิเล็กทรอนิกส์}
...
\subsubsection{งานพ่นสีและเคลือบผิว (Painting and Coating)}
...
\subsection{Machine Vision ร่วมกับหุ่นยนต์}

\subsubsection{บทบาทของ Machine Vision}
...
\subsubsection{องค์ประกอบของระบบ Machine Vision}
...
\subsubsection{ประเภทของ Camera Vision}

\begin{table}[htbp]
    \centering
    \small
    \begin{tabularx}{\textwidth}{L{0.14\textwidth} L{0.16\textwidth} Y Y L{0.18\textwidth}}
        \hline
...
\subsubsection{ความละเอียดเชิงพื้นที่}
...
\subsubsection{Robot Vision Workflow}
...
\subsubsection{ตัวอย่างเพิ่มเติมเพื่อการเรียนรู้: แปลงพิกัด 2D อย่างง่าย}
...
\subsection{Delta Robot และงาน Pick and Place ความเร็วสูง}
...
\subsection{ระบบสายพานลำเลียงและ Conveyor Tracking}

\subsubsection{หลักการพื้นฐาน}
...
\subsubsection{การแปลง Encoder Count เป็นระยะทาง}
...
\subsubsection{การชดเชยตำแหน่งชิ้นงาน}
...
\subsubsection{ตัวอย่างเพิ่มเติมเพื่อการเรียนรู้: Encoder Tracking}
...
\subsubsection{แหล่งความคลาดเคลื่อน}
...
\subsubsection{การตรวจสอบเชิงทดลอง}
...
\subsection{การบูรณาการระบบควบคุมและลำดับการทำงาน}

\subsubsection{สถาปัตยกรรมการควบคุมระดับ Cell}
...
\subsubsection{Handshake ที่ตรวจสอบได้}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{L{0.16\textwidth} Y}
        \hline
        \textbf{PLC \(\rightarrow\) Robot} & \textbf{ความหมาย} \\
        \hline
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l l}
        \hline
        \textbf{Robot \(\rightarrow\) PLC} & \textbf{ความหมาย} \\
        \hline
...
\subsubsection{State Machine ของ Robot Cell}
...
\subsubsection{Interlock ตัวอย่าง}
...
\subsubsection{Alarm และ Recovery}
...
\subsection{การวิเคราะห์ความเป็นไปได้ในการใช้หุ่นยนต์}

\subsubsection{งานแบบใดเหมาะกับหุ่นยนต์}
...
\subsubsection{ตารางประเมินเชิงคุณภาพ}
...
\subsubsection{การเลือกแนวทางอัตโนมัติ}
...
\subsection{การคำนวณประสิทธิภาพและผลตอบแทนการลงทุน}

\subsubsection{ต้นทุนที่ต้องพิจารณา}
...
\subsubsection{ผลประโยชน์ที่อาจเกิดขึ้น}
...
\subsubsection{ผลประหยัดสุทธิต่อปี}
...
\subsubsection{มูลค่าปัจจุบันสุทธิแบบเบื้องต้น}
...
\subsubsection{ตัวอย่างเพิ่มเติมเพื่อการเรียนรู้: ROI และ Payback}
...
\subsubsection{“Payload ผ่าน แปลว่าเลือกหุ่นยนต์ได้แน่นอน”}
...
\subsubsection{“Reach สูงสุดครอบคลุมจุดงาน จึงไม่มีปัญหา”}
...
\subsubsection{“Accuracy และ Repeatability เป็นค่าเดียวกัน”}
...
\subsubsection{“เพิ่มความเร็ว Robot แล้ว Cycle Time จะลดตามสัดส่วน”}
...
\subsubsection{“Machine Vision แก้ปัญหา Fixture ได้ทั้งหมด”}
...
\subsubsection{“กล้องมีจำนวน Megapixel สูงจึงแม่นยำกว่าเสมอ”}
...
\subsubsection{“Emergency Stop คือมาตรการความปลอดภัยหลักที่เพียงพอ”}
...
\subsubsection{“Simulation ตรงกับระบบจริง 100\%”}
...
\subsubsection{“ROI สูงหมายความว่าโครงการไม่มีความเสี่ยง”}
...
\subsubsection{“เมื่อ Robot Cell หยุด ให้ข้าม Interlock เพื่อเดินเครื่องเร็วขึ้น”}
...
\subsection{การประยุกต์ใช้ในงานจริงและกรณีศึกษา}

\subsubsection{กรณีศึกษา A: Robot Cell ประกอบชิ้นส่วนพลาสติกสองชิ้น}
...
\subsubsection{กรณีศึกษา B: Palletizing กล่องจากสายพาน}
...
\subsubsection{กรณีศึกษา C: Vision-Guided Pick บนสายพาน}
...
การออกแบบเริ่มจาก Task Analysis \(\rightarrow\) Robot/EOAT Selection \(\rightarrow\) Layout \(\rightarrow\) Control Sequence \(\rightarrow\) Safety \(\rightarrow\) Cycle Time \(\rightarrow\) Economic Evaluation ทุกสมมติฐานต้องแสดงที่มาและแยกระหว่างข้อมูลกำหนดกับค่าประมาณของกลุ่ม
...
\textbf{ตัวอย่างสัญญาณ PLC \(\rightarrow\) Robot:} \texttt{CELL\_AUTO}, \texttt{SAFETY\_OK}, \texttt{PART\_A\_READY}, \texttt{FIXTURE\_B\_READY}, \texttt{START\_REQ}, \texttt{RESET\_REQ}\\
\textbf{ตัวอย่างสัญญาณ Robot \(\rightarrow\) PLC:} \texttt{ROBOT\_READY}, \texttt{ROBOT\_BUSY}, \texttt{GRIP\_OK}, \texttt{CYCLE\_DONE}, \texttt{ROBOT\_HOME}, \texttt{ROBOT\_FAULT}

\textbf{ลำดับ:} PLC ตรวจความพร้อม \(\rightarrow\) ยก \texttt{START\_REQ} \(\rightarrow\) Robot ยก \texttt{ROBOT\_BUSY} \(\rightarrow\) Robot หยิบ A \(\rightarrow\) ตรวจ \texttt{GRIP\_OK} \(\rightarrow\) วาง B \(\rightarrow\) ยก \texttt{CYCLE\_DONE} \(\rightarrow\) PLC ลด \texttt{START\_REQ} \(\rightarrow\) Robot ลด \texttt{CYCLE\_DONE}
...
\begin{enumerate}
    ...
    \item \textbf{Sequence:} ตรวจ Cell Safe \(\rightarrow\) Vision Locate A/B \(\rightarrow\) Pick A \(\rightarrow\) Place Fixture \(\rightarrow\) Pick B \(\rightarrow\) Align \(\rightarrow\) Force-controlled Insert \(\rightarrow\) Verify Force/Position \(\rightarrow\) Release \(\rightarrow\) Mark Result \(\rightarrow\) Output
...
\begin{table}[htbp]
    \centering
    \scriptsize
    \begin{tabularx}{\textwidth}{L{0.20\textwidth} L{0.13\textwidth} Y Y Y}
        \hline
...
\begin{table}[htbp]
    \centering
    \scriptsize
    \begin{tabularx}{\textwidth}{L{0.14\textwidth} L{0.18\textwidth} Y Y L{0.16\textwidth}}
        \hline
